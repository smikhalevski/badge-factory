export const HttpMethod = {
   ...
   * is not defined by this specification, but might be defined by future extensions to HTTP. Content negotiation MAY
   * be
   ...
   * a
   * Max-Forwards field. If the Max-Forwards field-value is zero ("0"), the proxy MUST NOT forward the message,
   * instead,
   ...
   * might be a data-accepting process, a gateway to some other protocol, or a separate entity that accepts
   * annotations.
   ...
   * that the request be applied to a different URI, it MUST send a 301 (Moved Permanently) response, the user agent
   * MAY
   ...
   * length of the request chain, which is useful for testing a chain of proxies forwarding messages in an infinite
   * loop.
   *

...
export const HttpStatusCode = {
   ...
   * status code is received in response before continuing (or receive 417 Expectation Failed and not continue).[6]
   * @type {Number}
   */
  CONTINUE: 100,
   ...
   * Switching Protocols
   *
   * This means the requester has asked the server to switch protocols and the server is acknowledging that it will do
   * so.[7]
   * @type {Number}
   */
   ...
   * available yet.[8] This prevents the client from timing out and assuming the request was lost.
   * @type {Number}
   */
  PROCESSING: 102,
   ...
   * response will contain an entity describing or containing the result of the action.[10]
   * @type {Number}
   */
  OK: 200,
   ...
   * might not eventually be acted upon, as it might be disallowed when processing actually takes place.[12]
   * @type {Number}
   */
  ACCEPTED: 202,
   ...
   * response requires that the requester reset the document view.[15]
   * @type {Number}
   */
  RESET_CONTENT: 205,
   ...
   * multiple simultaneous streams.[16]
   * @type {Number}
   */
  PARTIAL_CONTENT: 206,
   ...
   * how many sub-requests were made.[17]
   * @type {Number}
   */
  MULTI_STATUS: 207,
   ...
   * more instance-manipulations applied to the current instance.[19]
   * @type {Number}
   */
  IM_USED: 226,
   ...
   * Multiple Choices
   *
   * Indicates multiple options for the resource that the client may follow. It, for instance, could be used to present
   * different format options for video, list files with different extensions, or word sense disambiguation.[22]
   * @type {Number}
   */
   ...
   * the client to perform a temporary redirect (the original describing phrase was "Moved Temporarily"),[24] but
   * popular browsers implemented 302 with the functionality of a 303 See Other. Therefore, HTTP/1.1 added status codes
   * 303 and 307 to distinguish between the two behaviours.[25] However, some Web applications and frameworks use the
   * 302 status code as if it were the 303.[26]
   * @type {Number}
   */
  FOUND: 302,
   ...
   *
   * The response to the request can be found under another URI using a GET method. When received in response to a POST
   * (or PUT/DELETE), it should be assumed that the server has received the data and the redirect should be issued with
   * a separate GET message.[27]
   * @type {Number}
   */
  SEE_OTHER: 303,
   ...
   * If-Modified-Since or If-None-Match. This means that there is no need to retransmit the resource, since the client
   * still has a previously-downloaded copy.[28]
   * @type {Number}
   */
  NOT_MODIFIED: 304,
   ...
   * clients (such as Mozilla[29] and Internet Explorer) do not correctly handle responses with this status code,
   * primarily for security reasons.[30]
   * @type {Number}
   */
  USE_PROXY: 305,
   ...
   * when reissuing the original request. For instance, a POST request should be repeated using another POST
   * request.[32]
   * @type {Number}
   */
   ...
   * permanently redirected resource may continue smoothly.[33]
   * @type {Number}
   */
   ...
   * malformed request syntax, invalid request message framing, or deceptive request routing).[36]
   * @type {Number}
   */
  BAD_REQUEST: 400,
   ...
   * requested resource. See Basic access authentication and Digest access authentication.[37]
   * @type {Number}
   */
  UNAUTHORIZED: 401,
   ...
   * authenticating will make no difference.[39]
   * @type {Number}
   */
  FORBIDDEN: 403,
   ...
   * modifications at any later time."[43]
   * @type {Number}
   */
  REQUEST_TIMEOUT: 408,
   ...
   * the case of multiple updates.[44]
   * @type {Number}
   */
  CONFLICT: 409,
   ...
   * code, the client should not request the resource again in the future. Clients such as search engines should remove
   * the resource from their indices.[45] Most use cases do not require clients and search engines to purge the
   ...
   *
   * The request is larger than the server is willing or able to process. Called "Request Entity Too Large"
   * previously.[48]
   * @type {Number}
   */
   ...
   * query-string of a GET request, in which case it should be converted to a POST request.[49] Called "Request-URI Too
   * Long" previously.[50]
   * @type {Number}
   */
  URI_TOO_LONG: 414,
   ...
   * example, if the client asked for a part of the file that lies beyond the end of the file.[51] Called "Requested
   * Range Not Satisfiable" previously.[52]
   * @type {Number}
   */
   ...
   * should be returned by tea pots requested to brew coffee.[54] This HTTP status is used as an easter egg in some
   * websites, including Google.com.[55]
   * @type {Number}
   */
  IM_A_TEAPOT: 418,
   ...
   * clients being denied access to specific server resources.[citation needed][56]
   * @type {Number}
   */
   ...
   *
   * Not part of the HTTP standard, but defined by Spring in the HttpStatus class to be used when a method failed. This
   * status code is deprecated by Spring.[57]
   * @type {Number}
   */
  METHOD_FAILURE: 420,
   ...
   *
   * Not part of the HTTP standard, but returned by version 1 of the Twitter Search and Trends API when the client is
   * being rate limited.[58] Other services may wish to implement the 429 Too Many Requests response code instead.
   * @type {Number}
   */
   ...
   * modified the state on the server, leading to a conflict."[61]
   * @type {Number}
   */
   ...
   * (useful as a deterrent for malware).[63]
   * @type {Number}
   */
  NO_RESPONSE: 444,
   ...
   *
   * A Microsoft extension. This error is given when Windows Parental Controls are turned on and are blocking access to
   * the given webpage.[65]
   * @type {Number}
   */
   ...
   *
   * Defined in the internet draft "A New HTTP Status Code for Legally-restricted Resources".[66] Intended to be used
   * when resource access is denied for legal reasons, e.g. censorship or government-mandated blocked access. A
   * reference to the 1953 dystopian novel Fahrenheit 451, where books are outlawed.[67]
   * @type {Number}
   */
   ...
   * users' mailbox.[68] The client is supposed to re-run the HTTP Autodiscovery protocol to find a better suited
   * server.[69]
   * @type {Number}
   */
  REDIRECT: 451,
   ...
   *
   * Nginx internal code similar to 431 but it was introduced earlier in version 0.9.4 (on January 21,
   * 2011).[70][original research?]
   * @type {Number}
   */
   ...
   * log and an error page redirection.[73]
   * @type {Number}
   */
  HTTP_TO_HTTPS: 497,
   ...
   * its request, making server unable to send a status code back.[75]
   * @type {Number}
   */
   ...
   * this implies future availability (e.g., a new feature of a web-service API).[79]
   * @type {Number}
   */
  NOT_IMPLEMENTED: 501,
   ...
   * Internet access via a Wi-Fi hotspot).[61]
   * @type {Number}
   */
   ...
   * Unknown Error
   *
   * This status code is not specified in any RFC and is returned by certain services, for instance Microsoft Azure and
   * CloudFlare servers: "The 520 error is essentially a �catch-all� response for when the origin server returns
   * something unexpected or something that is not tolerated/interpreted (protocol violation or empty response)."[87]
   * @type {Number}
   */
  UNKNOWN_ERROR: 520,
   ...
   *
   * This status code is not specified in any RFCs, but is used by CloudFlare's reverse proxies to signal that a server
   ...
   * timeout behind the proxy to a client in front of the proxy.[citation needed][88]
   * @type {Number}
   */
   ...
   * timeout behind the proxy to a client in front of the proxy.[citation needed][89]
   * @type {Number}
   */
